...
function eupagopagaqui_config() {
    $configarray = array(
        "FriendlyName" => array("Type" => "System", "Value" => "euPago - Pagaqui"),
        "chave_api_pa" => array("FriendlyName" => "Chave API", "Type" => "text", "Size" => "20","Chave Cedida pelo euPago",),
...
function eupagopagaqui_link($params) {
    ...
    $gatewaychave_api = $params['chave_api_pa'];
...
if($data['referencia']!="" ){
    ...
    $result->valor= $amount;
        ...
        $result = geraReferenciaPQ($gatewaychave_api, $invoiceid, $amount);
...
<tr>
	<td colspan="2" align="center">
		<img src="https://seguro.eupago.pt/repositorio/imagens/eupagopagaqui.png" alt="euPago pagaqui" height="20" />
	</td>
</tr>
...
<tr>
    ...
    # Insere os dados de pagaqui numa tabela para posterior valida��o
    insert_references_on_database_pa($invoiceid,'', $result->referencia, $result->valor, $invoiceid);
...
function insert_references_on_database_pa($invoiceid, $entidade, $referencia, $valor, $orderid) {
  $tabela = "tbleupago_pagaqui";
  

    check_table_exist_pa($tabela);
...
function check_table_exist_pa($tabela) {
